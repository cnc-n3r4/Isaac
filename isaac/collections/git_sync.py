...
class GitSync:
    ...
    def repo_root(self) -> Optional[Path]:
        try:
            out = subprocess.check_output(
                ["git", "rev-parse", "--show-toplevel"], cwd=str(self.repo_path), text=True
            ).strip()
            return Path(out)
        except Exception:
            return None
    ...
    def current_branch(self) -> Optional[str]:
        try:
            out = subprocess.check_output(
                ["git", "rev-parse", "--abbrev-ref", "HEAD"], cwd=str(self.repo_path), text=True
            ).strip()
            return out
        except Exception:
            return None
    ...
    def diff_files(self) -> List[str]:
        ...
        try:
            out = subprocess.check_output(
                ["git", "status", "--porcelain"], cwd=str(self.repo_path), text=True
            )
            files = []
