...
class XaiClient:
    ...
    def _call_api_with_system_prompt(
        self, system_prompt: str, user_prompt: str, max_tokens: int = 1024, temperature: float = 0
    ) -> Dict:
        """
    ...
    def _call_api_with_messages(
        self, messages: list, max_tokens: int = 1024, temperature: float = 0
    ) -> Dict:
        """
    ...
    def chat(self, prompt: str, system_prompt: Optional[str] = None) -> str:
        ...
        if system_prompt:
            result = self._call_api_with_system_prompt(
                system_prompt=system_prompt, user_prompt=prompt, max_tokens=1024, temperature=0.7
            )
        else:
            result = self._call_api(prompt=prompt, max_tokens=1024, temperature=0.7)
...
Only respond with JSON, no other text."""
        ...
        try:
            ...
            return {"safe": False, "warnings": ["AI validation failed"], "suggestions": []}
...
Only respond with JSON, no other text."""
        ...
        if not result["success"]:
            ...
            return {"corrected": None, "original": command, "confidence": 0.0}
        ...
        try:
            ...
            if has_typo:
                ...
                return {"corrected": None, "original": command, "confidence": 1.0}
        except json.JSONDecodeError:
            return {"corrected": None, "original": command, "confidence": 0.0}
...
Only respond with JSON, no other text."""
        ...
        try:
            ...
            return {"success": False, "error": "Failed to parse task plan"}
