...
class RoutingConfigManager:
    ...
    DEFAULT_CONFIG = {
        ...
        "providers": {
            "grok": {
                ...
                "pricing": {"input_per_1m": 5.0, "output_per_1m": 15.0, "currency": "USD"},
                "speed": "fast",
            ...
            "openai": {
                ...
                "pricing": {"input_per_1m": 0.15, "output_per_1m": 0.60, "currency": "USD"},
                "speed": "fast",
                "context_window": 128000,
            },
        },
    ...
    def update_provider_pricing(
        self, provider: str, input_per_1m: float, output_per_1m: float
    ) -> None:
    ...
    def get_enabled_providers(self) -> list:
        ...
        return [
            name for name, config in self.config["providers"].items() if config.get("enabled", True)
        ]
    ...
    def validate_config(self) -> tuple[bool, list]:
        ...
        for complexity, provider in self.config["routing_preferences"].items():
            if complexity in ["simple", "medium", "complex", "expert"]:
                if provider not in self.config["providers"]:
                    errors.append(
                        f"Routing preference '{complexity}' points to unknown provider '{provider}'"
                    )
                elif not self.config["providers"][provider].get("enabled", True):
                    errors.append(
                        f"Routing preference '{complexity}' points to disabled provider '{provider}'"
                    )
