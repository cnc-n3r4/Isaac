...
def translate_query(query: str, shell_name: str, session_mgr) -> Dict:
    ...
    try:
        ...
        return {"success": False, "error": f"Failed to initialize AI router: {str(e)}"}
...
Only respond with the JSON, no other text."""
    ...
    if not response.success:
        return {"success": False, "error": f"AI translation failed: {response.error}"}
    ...
    if not command or len(command) == 0:
        return {"success": False, "error": "AI returned empty command"}
