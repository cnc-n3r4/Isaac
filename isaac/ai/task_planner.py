...
def execute_task(task_description: str, shell, session_mgr) -> CommandResult:
    ...
    if not session_mgr.config.get("task_mode_enabled", True):
        return CommandResult(
            success=False, output="Isaac > Task mode disabled in config", exit_code=-1
        )
    ...
    if not api_key:
        return CommandResult(
            success=False, output="Isaac > xAI API key not configured", exit_code=-1
        )
    ...
    try:
        ...
        client = XaiClient(api_key=api_key, model=model, api_url=api_url, timeout=timeout)
    except Exception as e:
        return CommandResult(
            success=False, output=f"Isaac > Failed to initialize AI: {str(e)}", exit_code=-1
        )
    ...
    if not steps:
        return CommandResult(
            success=False, output="Isaac > No steps generated for task", exit_code=-1
        )
    ...
    if mode == "4":
        return CommandResult(success=False, output="Isaac > Task aborted by user", exit_code=-1)

    execution_mode = {"1": "autonomous", "2": "approve-once", "3": "step-by-step"}.get(
        mode, "step-by-step"
    ...
    task_id = session_mgr.task_history.create_task(
        description=task_description, steps=steps, mode=execution_mode
    )
    ...
    for i, step in enumerate(steps, 1):
        ...
        if execution_mode == "step-by-step":
            ...
            if confirm == "abort":
                ...
                return CommandResult(
                    success=False, output=f"Isaac > Task aborted at step {i}", exit_code=-1
                )
            elif confirm != "y":
    ...
    return CommandResult(
        success=True, output=f"Isaac > Task complete: {len(steps)} steps executed", exit_code=0
    )


def _handle_step_failure(
    task_id: str, step_num: int, step: Dict, error: str, client: XaiClient, shell, session_mgr
) -> str:
    """
    ...
    if choice == "1":
        ...
        # Skip
        session_mgr.task_history.log_step(
            task_id, step_num, "skipped", f"Skipped after failure: {error}"
        )
        session_mgr.save_task_history()
        return "skip"
...
def _get_ai_fix_suggestion(command: str, error: str, shell_name: str, client: XaiClient) -> Dict:
    """
...
Only respond with JSON."""
    ...
    if not result.get("success"):
        return {"fixed_command": command, "explanation": "AI unavailable, no fix suggested"}
    ...
    try:
        ...
        return {"fixed_command": command, "explanation": "Failed to parse AI response"}
