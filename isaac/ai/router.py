...
class AIRouter:
    ...
    def chat(
        ...
        **kwargs
        ...
        if prefer_provider is None:
            ...
            estimated_tokens = task_analysis.get('estimated_tokens', {'input': 1000, 'output': 1000})
        else:
        ...
        for provider in fallback_order:
            ...
            try:
                ...
                if response.success:
                    ...
                    task_type = task_analysis.get('task_type', 'unknown') if task_analysis else 'unknown'
                    ...
                    response.metadata.update({
                        'routing': {
                            ...
                            'complexity': task_analysis.get('complexity') if task_analysis else 'unknown',
                            'task_type': task_type,
                            'reasoning': task_analysis.get('reasoning') if task_analysis else 'User preference',
    ...
    def analyze_task_preview(self, messages: List[Dict[str, str]], tools: Optional[List[Dict[str, Any]]] = None) -> Dict[str, Any]:
        """
    ...
    def check_budget_health(self) -> Dict[str, Any]:
        ...
        if budget_status['daily']['status'] == 'warning':
            recommendations.append("Daily budget at 80%. Consider simpler queries or cheaper providers.")

        if budget_status['daily']['status'] == 'exceeded':
            recommendations.append("Daily budget exceeded. Requests will use cheapest available providers.")

        if not forecast['on_track']:
            recommendations.append(f"Monthly forecast: ${forecast['projected_total']:.2f} (over budget). Reduce usage or increase limits.")
        ...
        for provider, cost in report['provider_totals'].items():
            if cost > report['total_cost'] * 0.7:  # One provider is >70% of costs
                recommendations.append(f"{provider.title()} is {cost/report['total_cost']*100:.0f}% of costs. Consider routing simple tasks elsewhere.")
    ...
    def _calculate_budget_health_score(self, budget_status: Dict[str, Any], forecast: Dict[str, Any]) -> str:
        """Calculate overall budget health score"""
        if budget_status['daily']['status'] == 'exceeded' or budget_status['monthly']['status'] == 'exceeded':
            return 'critical'
