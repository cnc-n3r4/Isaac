...
class EnhancedAntiPatternDetector:
    ...
    def analyze_file(self, file_path: str, content: Optional[str] = None) -> CodeQualityReport:
        ...
        for ap in report.anti_patterns:
            report.issues_by_category[ap.category] = (
                report.issues_by_category.get(ap.category, 0) + 1
            )
            report.issues_by_severity[ap.severity] = (
                report.issues_by_severity.get(ap.severity, 0) + 1
            )
            if ap.can_auto_fix:
    ...
    def generate_fix_script(self, report: CodeQualityReport) -> str:
        ...
        for i, issue in enumerate(fixable_issues):
            if issue.auto_fix_code:
                script_lines.extend(
                    [f"    # Fix {i+1}: {issue.rule_name}", f"    {issue.auto_fix_code}", ""]
                )
    ...
    def _analyze_function(
        self, node: ast.FunctionDef, content: str, lines: List[str]
    ) -> List[AntiPatternDetection]:
        ...
        for rule_id, rule in self.rules.items():
            ...
            if rule.condition_checker:
                try:
                    ...
                    if result:
                        detection = self._create_detection_from_result(
                            rule, result, node, content, lines
                        )
                        if detection:
    ...
    def _analyze_class(
        self, node: ast.ClassDef, content: str, lines: List[str]
    ) -> List[AntiPatternDetection]:
    ...
    def _analyze_module(
        self, node: ast.Module, content: str, lines: List[str]
    ) -> List[AntiPatternDetection]:
    ...
    def _create_detection_from_result(
        self,
    ...
    def _deduplicate_anti_patterns(
        self, anti_patterns: List[AntiPatternDetection]
    ) -> List[AntiPatternDetection]:
    ...
    def _calculate_quality_score(self, report: CodeQualityReport) -> float:
        ...
        severity_penalties = {"critical": 20, "high": 10, "medium": 5, "low": 2, "info": 0.5}
    ...
    def _calculate_maintainability_index(self, report: CodeQualityReport) -> float:
        ...
        factors = {
            "function_length": len(
                [ap for ap in report.anti_patterns if "long" in ap.rule_name.lower()]
            ),
            "complexity": len(
                [ap for ap in report.anti_patterns if "complex" in ap.rule_name.lower()]
            ),
            "documentation": len(
                [ap for ap in report.anti_patterns if "docstring" in ap.rule_name.lower()]
            ),
            "structure": len(
                [ap for ap in report.anti_patterns if "structure" in ap.rule_name.lower()]
            ),
        }
    ...
    def _check_missing_docstring(
        self, node: ast.AST, content: str, lines: List[str]
    ) -> Dict[str, Any]:
    ...
    def _check_imports_not_at_top(
        ...
        for node in tree.body:
            ...
            elif not isinstance(node, (ast.Expr, ast.Constant)) and not (
                hasattr(ast, "Str") and isinstance(node, ast.Str)
            ):  # Skip docstrings and strings
