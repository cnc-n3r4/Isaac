...
class PatternDocumentationGenerator:
    ...
    def generate_documentation(
        self, pattern_data: Dict[str, Any], usage_stats: Optional[Dict[str, Any]] = None
        ...
        documentation = PatternDocumentation(pattern_id=pattern_id, **filtered_doc_data)
    ...
    def generate_index(self, category: Optional[str] = None) -> str:
        ...
        for cat_name, cat_patterns in categories.items():
            ...
            for pattern in cat_patterns:
                usage_indicator = (
                    f" (Used {pattern.usage_count} times)" if pattern.usage_count > 0 else ""
                )
                lines.append(
                    f"- [**{pattern.title}**]({pattern.pattern_id}.md) - {pattern.summary}{usage_indicator}"
                )
    ...
    def search_documentation(self, query: str) -> List[PatternDocumentation]:
        ...
        results.sort(key=lambda d: (d.title.lower().find(query_lower), -d.usage_count))
...
""",
    ...
    def _generate_content_sections(
        self, pattern_data: Dict[str, Any], template: DocumentationTemplate
    ) -> Dict[str, Any]:
    ...
    def _fill_template(self, template: str, doc: PatternDocumentation) -> str:
        ...
        template_vars["examples"] = (
            self._format_examples(doc.examples) if doc.examples else "*No examples available*"
        )
