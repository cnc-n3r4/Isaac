...
def execute_debug(args: Dict[str, Any]) -> str:
    ...
    try:
        if mode == 'full':
            ...
            return f"❌ Unknown debug mode: {mode}. Use: full, quick, performance, tests, or history"
...
def format_full_debug_output(result: Dict[str, Any]) -> str:
    ...
    if root_cause:
        ...
        if hypotheses:
            ...
            for i, hyp in enumerate(hypotheses[:3], 1):
                output_lines.append(f"  {i}. {hyp.get('description', 'Unknown')} "
    ...
    if historical:
        ...
        if similar_solutions:
            ...
            output_lines.append(f"Top historical solution (confidence: {top_solution['confidence']:.1%}):")
        ...
        if command_stats:
            ...
            output_lines.append(f"Command debug stats: {success_rate:.1%} success rate, "
