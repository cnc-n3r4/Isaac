...
class BubbleCommand:
    ...
    def _create_bubble(self, args: List[str]) -> Dict[str, Any]:
        ...
        try:
            bubble = self.bubble_manager.create_bubble(
                name=parsed.name, description=parsed.description or "", tags=parsed.tags or []
            )
            ...
            return {"success": False, "output": f"Failed to create bubble: {e}", "exit_code": 1}
    ...
    def _list_bubbles(self, args: List[str]) -> Dict[str, Any]:
        ...
        if not bubbles:
            return {"success": True, "output": "No bubbles found.", "exit_code": 0}
    ...
    def _resume_bubble(self, args: List[str]) -> Dict[str, Any]:
        """Resume a bubble."""
        if not args:
            return {"success": False, "output": "Usage: /bubble resume <bubble_id>", "exit_code": 1}

        bubble_id = args[0]
        success = self.bubble_manager.resume_bubble(bubble_id)

        if success:
            return {"success": True, "output": f"Resumed bubble '{bubble_id}'", "exit_code": 0}
        else:
            return {
                "success": False,
    ...
    def _suspend_bubble(self, args: List[str]) -> Dict[str, Any]:
        ...
        if success:
            return {"success": True, "output": f"Suspended bubble '{bubble_id}'", "exit_code": 0}
        else:
            return {
                "success": False,
    ...
    def _delete_bubble(self, args: List[str]) -> Dict[str, Any]:
        """Delete a bubble."""
        if not args:
            return {"success": False, "output": "Usage: /bubble delete <bubble_id>", "exit_code": 1}
        ...
        if success:
            return {"success": True, "output": f"Deleted bubble '{bubble_id}'", "exit_code": 0}
        else:
            return {"success": False, "output": f"Bubble '{bubble_id}' not found", "exit_code": 1}
    ...
    def _show_bubble_info(self, args: List[str]) -> Dict[str, Any]:
        ...
        if not args:
            return {"success": False, "output": "Usage: /bubble info <bubble_id>", "exit_code": 1}
        ...
        if bubble.git_state.get("is_git_repo"):
            ...
            if bubble.git_state.get("recent_commits"):
                output += (
                    f"  Recent commits: {len(bubble.git_state['recent_commits'].splitlines())}\n"
                )
    ...
    def _show_current_bubble(self) -> Dict[str, Any]:
        ...
        if not current:
            return {"success": True, "output": "No current bubble active", "exit_code": 0}
