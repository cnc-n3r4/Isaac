...
def _handle_chat_query(
    query: str, config: dict, session: SessionManager, is_piped_input: bool = False
) -> str:
    ...
    if not api_key:
        raise Exception(
            "xAI Chat API key not configured. Set it in ~/.isaac/config.json under xai.chat.api_key"
        )
    ...
    # Query AI
    return client.chat(prompt=query, system_prompt=preprompt)
    ...
    try:
        ...
        if not sys.stdin.isatty():
            if return_blob:
                ...
                print(
                    json.dumps(
                        {"kind": "error", "content": f"Error: {e}", "meta": {"command": command}}
                    )
                )
            else:
                # Dispatcher call - return envelope error
...
def _build_chat_preprompt(
    session: SessionManager, current_query: str, is_piped_input: bool = False
) -> str:
    """
