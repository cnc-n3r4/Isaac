...
class PipelineCommand:
    ...
    def _create_pipeline(self, args: List[str]) -> Dict[str, Any]:
        ...
        try:
            if parsed.template:
                ...
                pipeline = Pipeline(
                    id=pipeline_id, name=parsed.name, description=parsed.description or ""
                )
            ...
            return {"success": False, "output": f"Error creating pipeline: {e}", "exit_code": 1}
    ...
    def _show_status(self, args: List[str]) -> Dict[str, Any]:
        ...
        if not args:
            ...
            if not executions:
                return {"success": True, "output": "No active pipeline executions", "exit_code": 0}
    ...
    def _delete_pipeline(self, args: List[str]) -> Dict[str, Any]:
        ...
        if self.manager.delete_pipeline(pipeline_id):
            return {"success": True, "output": f"Deleted pipeline '{pipeline_id}'", "exit_code": 0}
        else:
            return {
                "success": False,
    ...
    def _show_suggestions(self, args: List[str]) -> Dict[str, Any]:
        ...
        for template in templates:
            ...
            output += (
                f"  Create with: /pipeline create \"My {template['name']}\" -t {template['id']}\n\n"
            )
    ...
    def _import_pipeline(self, args: List[str]) -> Dict[str, Any]:
        ...
        if not import_path.exists():
            return {"success": False, "output": f"File not found: {import_path}", "exit_code": 1}
