...
class ProactiveSuggester:
    ...
    def _clean_expired_suggestions(self) -> None:
        ...
        expired_ids = [
            sid for sid, suggestion in self.suggestions.items()
    ...
    def _generate_error_suggestions(self, command: str, output: str) -> None:
        ...
        if 'command not found' in output.lower():
            suggestion = Suggestion(
                ...
                actions=[
                    {'label': 'Search for similar commands', 'command': f'/help search {command.split()[0]}'},
            ...
            suggestion = Suggestion(
                ...
                actions=[
                    ...
                    {'label': 'Check permissions', 'command': f'ls -la {command.split()[-1] if len(command.split()) > 1 else "."}'}
                ]
            )
        else:
        ...
        for i, pattern_sug in enumerate(pattern_suggestions):
            ...
            if pattern_sug['type'] == 'pipeline_suggestion':
                suggestion = Suggestion(
                    ...
                    timestamp=time.time(),
                    expires_at=time.time() + self.suggestion_timeout * 2,  # Longer timeout for pipeline suggestions
                    actions=[
                        {'label': 'Create Pipeline', 'command': f'/pipeline create "Auto {command.split()[0]}" -t custom'},
                        {'label': 'View Pattern', 'command': f'/ambient patterns show {pattern_sug["pattern_id"]}'}
                    ]
                )
                ...
                if next_cmds:
                    suggestion = Suggestion(
                        ...
                        actions=[
                            ...
                            {'label': 'Show full sequence', 'command': f'/ambient patterns show {pattern_sug["pattern"]["pattern_id"]}'}
                        ]
                    )
                else:
                    continue
    ...
    def get_learning_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'current_error_rate': self.current_context['error_count'] / max(self.current_context['command_count'], 1)
        }