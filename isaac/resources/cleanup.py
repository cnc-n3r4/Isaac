...
class CleanupManager:
    ...
    def _save_history(self):
        ...
        data = {
            ...
            "history": [
                r.to_dict() if isinstance(r, CleanupResult) else r for r in self.cleanup_history
            ],
        }
    ...
    def cleanup_docker_images(self, dangling_only: bool = True) -> CleanupResult:
        ...
        try:
            if dangling_only:
                ...
                count = (
                    len(count_result.stdout.strip().split("\n"))
                ...
                result = subprocess.run(
                    ["docker", "image", "prune", "-f"], capture_output=True, text=True, timeout=30
                )
            else:
    ...
    def cleanup_docker_containers(self) -> CleanupResult:
        ...
        try:
            ...
            result = subprocess.run(
                ["docker", "container", "prune", "-f"], capture_output=True, text=True, timeout=30
            )
    ...
    def cleanup_docker_volumes(self) -> CleanupResult:
        ...
        try:
            ...
            # Prune volumes
            result = subprocess.run(
                ["docker", "volume", "prune", "-f"], capture_output=True, text=True, timeout=30
            )
    ...
    def cleanup_package_cache(self, package_manager: str) -> CleanupResult:
        ...
        try:
            result = subprocess.run(
                commands[package_manager], capture_output=True, text=True, timeout=60
            )
