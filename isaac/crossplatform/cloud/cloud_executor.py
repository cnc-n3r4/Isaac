...
class CloudExecutor:
    ...
    async def _execute_on_cloud(
        ...
        if self.cloud_provider == 'aws':
            return await self._execute_aws(command, workspace_id, environment, stream_output, execution_id)
        elif self.cloud_provider == 'gcp':
            return await self._execute_gcp(command, workspace_id, environment, stream_output, execution_id)
        elif self.cloud_provider == 'azure':
            return await self._execute_azure(command, workspace_id, environment, stream_output, execution_id)
        else:
            return await self._execute_generic(command, workspace_id, environment, stream_output, execution_id)
    ...
    async def execute_batch(
        ...
        if parallel:
            tasks = [
                self.execute_command(cmd, workspace_id)
                for cmd in commands
            ]
