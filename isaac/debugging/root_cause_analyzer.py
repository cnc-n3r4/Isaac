...
class RootCauseAnalyzer:
    ...
    def analyze_root_cause(
        ...
        analysis_depth: AnalysisDepth = AnalysisDepth.INTERMEDIATE,
    ) -> RootCauseAnalysis:
        ...
        primary_root_cause, confidence = self._determine_primary_root_cause(
            hypotheses, causal_graph
        )
        ...
        preventive_measures = self._generate_preventive_measures(
            primary_root_cause, systemic_issues
        )
    ...
    def _generate_hypotheses(
        self, error_message: str, diagnostic_data: Dict[str, Any]
    ...
    def _create_hypothesis_from_pattern(
        self,
        cause: str,
    ...
    def _calculate_hypothesis_confidence(
        self, cause: str, error_message: str, diagnostic_data: Dict[str, Any]
    ) -> float:
        ...
        if cause in ["memory_leak", "insufficient_ram"] and "memory" in error_message.lower():
            ...
            cause in ["incorrect_permissions", "wrong_user"]
            and "permission" in error_message.lower()
        ):
            confidence += 0.4
        ...
        if cause in ["service_down", "firewall_block"] and not network_status.get(
            "has_internet", True
        ):
            confidence += 0.2
    ...
    def _gather_contradicting_evidence(
        self, cause: str, diagnostic_data: Dict[str, Any]
    ) -> List[str]:
        ...
        if cause == "insufficient_ram" and memory_usage.get("percent", 0) < 50:
            evidence.append(
                f"Memory usage is only {memory_usage['percent']:.1f}%, suggesting sufficient RAM"
            )
        elif cause == "disk_full" and disk_usage.get("percent", 0) < 50:
            evidence.append(
                f"Disk usage is only {disk_usage['percent']:.1f}%, suggesting sufficient space"
            )

        return evidence

    def _build_causal_chain(
        self, cause: str, causal_links: List[Dict[str, Any]]
    ) -> List[CausalLink]:
    ...
    def _generate_diagnostic_hypotheses(
    ...
    def _determine_primary_root_cause(
        self, hypotheses: List[RootCauseHypothesis], causal_graph: List[CausalLink]
    ) -> Tuple[str, float]:
    ...
    def _identify_systemic_issues(
        self, diagnostic_data: Dict[str, Any], causal_graph: List[CausalLink]
    ) -> List[str]:
        ...
        if memory_percent > 85:
            issues.append(
                "Chronic memory pressure - consider increasing RAM or optimizing applications"
            )
    ...
    def _generate_preventive_measures(
        self, primary_root_cause: str, systemic_issues: List[str]
    ) -> List[str]:
    ...
    def _refine_hypotheses_deep(
        self, hypotheses: List[RootCauseHypothesis], diagnostic_data: Dict[str, Any]
    ) -> List[RootCauseHypothesis]:
