...
class AutoInvestigator:
    ...
    def investigate_error(
        self, command: str, error_output: str, exit_code: int, working_dir: str = None
    ) -> InvestigationResult:
    ...
    def _check_network_status(self) -> Dict[str, Any]:
        ...
        network_info = {"has_internet": False, "dns_working": False}
    ...
    def _gather_evidence(self, error_category: str, diagnostic: DiagnosticInfo) -> List[str]:
        ...
        if error_category == "command_error":
            if "PATH" in diagnostic.environment:
                evidence.append(
                    f"PATH contains {len(diagnostic.environment['PATH'].split(os.pathsep))} directories"
                )
    ...
    def _generate_recommendations(
        self, error_category: str, diagnostic: DiagnosticInfo
    ) -> List[str]:
    ...
    def _suggest_fixes(
        self, error_category: str, diagnostic: DiagnosticInfo, original_command: str
    ) -> List[Dict[str, Any]]:
            ...
            fixes.append(
                {
                    ...
                    "commands": [
                        f"chmod +x {original_command}",
                        f"sudo chown $USER {original_command}",
                    ],
                    "confidence": 0.7,
                }
            )
