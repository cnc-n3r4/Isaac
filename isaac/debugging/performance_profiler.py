...
class PerformanceProfiler:
    ...
    def stop_profiling(self) -> Optional[PerformanceProfile]:
        ...
        profile = self._analyze_performance(self.current_profile, end_time, final_metrics)
    ...
    def _capture_system_metrics(self) -> Dict[str, Any]:
        ...
        try:
            return {
                ...
                "network": {
                    "bytes_sent": (
                        psutil.net_io_counters().bytes_sent if psutil.net_io_counters() else 0
                    ),
                    "bytes_recv": (
                        psutil.net_io_counters().bytes_recv if psutil.net_io_counters() else 0
                    ),
                },
    ...
    def _analyze_performance(
        self, profile_data: Dict[str, Any], end_time: float, final_metrics: Dict[str, Any]
    ) -> PerformanceProfile:
    ...
    def _calculate_system_load(self, samples: List[Dict]) -> Dict[str, float]:
        ...
        avg_load = [sum(sample[i] for sample in load_samples) / len(load_samples) for i in range(3)]

        return {"1min": avg_load[0], "5min": avg_load[1], "15min": avg_load[2]}
    ...
    def _generate_recommendations(
        self, bottlenecks: List[str], command: str, duration: float
    ) -> List[str]:
    ...
    def compare_with_baseline(self, profile: PerformanceProfile) -> Dict[str, Any]:
        ...
        comparison = {
            ...
            "duration_change_percent": (
                (
                    (profile.duration - baseline.get("duration", 0))
                    / baseline.get("duration", 0)
                    * 100
                )
                if baseline.get("duration", 0)
                else 0
            ),
        ...
        if comparison["duration_change_percent"] > 20:
            ...
            comparison["message"] = (
                f"Performance degraded by {comparison['duration_change_percent']:.1f}%"
            )
        elif comparison["duration_change_percent"] < -20:
            comparison["assessment"] = "performance_improved"
            comparison["message"] = (
                f"Performance improved by {abs(comparison['duration_change_percent']):.1f}%"
            )
        else:
