...
class DebugCommand:
    ...
    def execute_debug_assistance(
        self, command: str, error_message: str = None, context: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            diagnostic_data = self.investigator.gather_diagnostic_data(
                command, error_message, context
            )
            ...
            result["fix_suggestions"] = {
                "primary_fix": (
                    asdict(fix_recommendations.primary_fix)
                    if fix_recommendations.primary_fix
                    else None
                ),
            ...
            try:
                ...
                if profile:
                    ...
                    result["performance_analysis"]["report"] = (
                        self.performance_profiler.generate_performance_report(profile)
                    )
            ...
            debug_session = DebugSession(
                ...
                solution=result["fix_suggestions"]
                .get("primary_fix", {})
                .get("commands", ["No solution found"])[0],
    ...
    def execute_quick_debug(self, command: str, error_message: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            result["immediate_fixes"] = {
                ...
                "alternative_fixes": [
                    asdict(fix) for fix in fix_recommendations.alternative_fixes[:2]
                ],  # Top 2 alternatives
            }
    ...
    def execute_test_generation(
        self, error_message: str, diagnostic_data: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            perf_tests = self.test_generator.generate_performance_tests(
                command, {"duration": 1.0, "cpu_usage": 50.0, "memory_usage": 60.0}
            )
    ...
    def get_debug_history_summary(self, days: int = 30) -> Dict[str, Any]:
        ...
        """
        result = {"period_days": days, "insights": [], "command_stats": {}, "pattern_summary": {}}
            ...
            with self.history_manager.db_path as db_path:
                ...
                with sqlite3.connect(db_path) as conn:
                    ...
                    for row in cursor.fetchall():
                        patterns.append({"error_pattern": row[0], "occurrences": row[1]})
    ...
    def _generate_debug_summary(self, debug_result: Dict[str, Any]) -> str:
        ...
        if similar_solutions:
            ...
            summary_parts.append(
                f"📚 Historical: {len(similar_solutions)} similar cases, "
