...
class FixSuggester:
            ...
            "disk_full": [
                {
                    ...
                    "category": "resource",
                    "confidence": 0.8,
                    "commands": ["sudo apt-get clean", "sudo yum clean all", "sudo pacman -Sc"],
    ...
    def _create_fix_suggestion(
        self, fix_data: Dict[str, Any], diagnostic_data: Dict[str, Any]
    ) -> FixSuggestion:
    ...
    def _interpolate_commands(
        self, commands: List[str], diagnostic_data: Dict[str, Any]
    ) -> List[str]:
        ...
        context_vars = {
            "command": (
                diagnostic_data.get("command", "").split()[0]
                if diagnostic_data.get("command")
                else ""
            ),
            "file_path": (
                diagnostic_data.get("related_files", [None])[0]
                if diagnostic_data.get("related_files")
                else ""
            ),
    ...
    def _generate_preventive_fixes(
        self, error_type: str, diagnostic_data: Dict[str, Any]
    ) -> List[FixSuggestion]:
    ...
    def validate_fix(self, fix: FixSuggestion) -> Dict[str, Any]:
        ...
        for prereq in fix.prerequisites:
            if prereq == "sudo access available":
                ...
                has_pm = any(
                    os.path.exists(f"/usr/bin/{pm}") or os.path.exists(f"/usr/local/bin/{pm}")
