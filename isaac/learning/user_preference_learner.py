...
class UserPreferences:
    ...
    def get_top_preferences(self, category: str, limit: int = 5) -> List[Tuple[str, CodingPattern]]:
        ...
        category_dict = getattr(self, category, {})
        sorted_patterns = sorted(category_dict.items(), key=lambda x: x[1].confidence, reverse=True)
...
class UserPreferenceLearner:
    ...
    def observe_command_usage(
        self, command: str, args: List[str], success: bool, context: Optional[Dict[str, Any]] = None
    ):
    ...
    def observe_ai_interaction(
        ...
        if user_feedback:
            ...
            self.observe_coding_pattern(
                "response_length", "preferred_length", len(ai_response.split()), context
            )
    ...
    def get_personalized_suggestion(
        self, suggestion_type: str, context: Dict[str, Any]
    ) -> Optional[Any]:
    ...
    def get_coding_style_recommendations(self, code_context: Dict[str, Any]) -> List[str]:
        ...
        for key, pattern in naming_prefs:
            if pattern.confidence > 0.7:
                recommendations.append(
                    f"Consider using {pattern.pattern_key} naming: {pattern.preference_value}"
                )
        ...
        for key, pattern in structure_prefs:
            if pattern.confidence > 0.7:
                recommendations.append(
                    f"Preferred {pattern.pattern_key}: {pattern.preference_value}"
                )
        ...
        for key, pattern in command_prefs:
            if pattern.confidence > 0.6:
                ...
                if isinstance(cmd_data, dict) and "command" in cmd_data:
                    suggestions.append(
                        f"You often use: {cmd_data['command']} {' '.join(cmd_data.get('args', []))}"
                    )
        ...
        for key, pattern in workflow_prefs:
            if pattern.confidence > 0.6:
                ...
                if isinstance(workflow_data, dict):
                    suggestions.append(
                        f"Workflow pattern: {workflow_data.get('operation', key)} on {workflow_data.get('extension', 'files')}"
                    )
    ...
    def _get_or_create_pattern(self, pattern_type: str, pattern_key: str) -> CodingPattern:
        ...
        if pattern_key not in self.learned_patterns[pattern_type]:
            self.learned_patterns[pattern_type][pattern_key] = CodingPattern(
                pattern_type=pattern_type, pattern_key=pattern_key, preference_value=None
            )
    ...
    def get_learning_stats(self) -> Dict[str, Any]:
        ...
        high_confidence_patterns = sum(
            1
            for category in self.learned_patterns.values()
