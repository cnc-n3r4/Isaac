...
class AnalyticsManager:
    ...
    def __init__(self, team_id: str = "default", db_path: Optional[str] = None):
    ...
    def record_code_change(self, file_path: str, change_type: str = "edit"):
    ...
    def record_pattern_learned(self, pattern_name: str, pattern_type: str, confidence: float):
        """Record newly learned pattern"""
        if not self.enabled:
            return

        self.learning.record_pattern_learned(
            pattern_name=pattern_name, pattern_type=pattern_type, confidence=confidence
        )
    ...
    def record_team_contribution(
        self, user_id: str, contribution_type: str, contribution_value: float = 1.0
    ):
