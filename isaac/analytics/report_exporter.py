...
class ReportExporter:
    ...
    def _export_csv(self, data: Dict[str, Any], report_type: str) -> str:
        ...
        if report_type == "productivity" and "breakdown" in data:
            ...
            for metric_type, metric_data in data["breakdown"].items():
                writer.writerow(
                    [metric_type, metric_data.get("count", 0), metric_data.get("total_value", 0)]
                )
            writer.writerow([])
    ...
    def _export_html(self, data: Dict[str, Any], report_type: str) -> str:
        ...
        if "period" in data:
            html.append(
                f'<p><strong>Period:</strong> {data["period"].get("start", "N/A")[:10]} to {data["period"].get("end", "N/A")[:10]}</p>'
            )
        html.append("</div>")
    ...
    def export_dashboard(
        self, dashboard_id: str, format: str = "html", output_path: Optional[str] = None
    ) -> str:
    ...
    def _export_dashboard_html(self, builder, dashboard_id: str) -> str:
        ...
        html.append("</div>")
        html.append(
            f'<p style="text-align: center; margin-top: 30px; color: #666;">Last updated: {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}</p>'
        )
        html.append("</body>")
        html.append("</html>")
    ...
    def schedule_report(
        ...
        config_file = os.path.join(config_dir, f"{report_type}_{schedule}.json")
