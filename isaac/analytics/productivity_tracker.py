...
class ProductivityTracker:
    ...
    def _load_baseline_metrics(self) -> Dict[str, float]:
        ...
        try:
            ...
            return {"avg_commands_per_session": 10.0, "avg_time_saved": 0.0, "avg_efficiency": 50.0}
    ...
    def record_command_execution(
        ...
        if estimated_manual_time:
            ...
            self.db.record_productivity_metric(
                ...
                metadata=json.dumps(
                    {"execution_time": execution_time, "manual_time": estimated_manual_time}
                ),
            )
    ...
    def record_automation_run(self, automation_name: str, steps_automated: int, time_saved: float):
    ...
    def record_pattern_application(self, pattern_name: str, lines_of_code_saved: int):
    ...
    def record_error_prevention(self, error_type: str, time_saved: float):
    ...
    def record_suggestion_accepted(self, suggestion_type: str, time_saved: float):
    ...
    def calculate_efficiency_score(self) -> float:
        ...
        command_factor = min(100, (self.commands_executed / (session_duration / 60)) * 10)
    ...
    def get_productivity_report(
        ...
        metrics = self.db.query_metrics(
            "productivity_metrics", start_date=start_date, end_date=end_date
        )
        ...
        for metric in metrics:
            ...
            if metric_type not in aggregated:
                aggregated[metric_type] = {"count": 0, "total_value": 0.0, "items": []}
        ...
        productivity_gain = (hours_saved / (days_elapsed * 8)) * 100  # Percentage of 8-hour workday

        return {
            "period": {"start": start_date, "end": end_date, "days": days_elapsed},
            "summary": {
    ...
    def _generate_insights(self, aggregated: Dict[str, Any], total_time_saved: float) -> List[str]:
        ...
        if error_count > 0:
            insights.append(f"🛡️  Prevented {error_count} potential errors before they occurred")

        # Efficiency insights
        if not insights:
            insights.append("📊 Continue using Isaac to build up productivity metrics")
    ...
    def get_efficiency_trend(self, days: int = 7) -> List[Dict[str, Any]]:
        ...
        metrics = self.db.query_metrics("productivity_metrics", start_date=start_date)
        ...
        for date, metrics in sorted(daily_metrics.items()):
            ...
            trend.append({"date": date, "efficiency": efficiency, **metrics})
