...
class LearningTracker:
    ...
    def record_pattern_learned(
        self, pattern_name: str, pattern_type: str, confidence: float, usage_count: int = 0
    ):
    ...
    def record_preference_adaptation(
        self, preference_name: str, preference_value: Any, confidence: float
    ):
    ...
    def record_mistake_learned(
        self, mistake_type: str, correction: str, success_rate_improvement: float
    ):
    ...
    def record_behavior_adjustment(
        self, behavior_name: str, adjustment_type: str, effectiveness: float
    ):
    ...
    def record_skill_improvement(
        self, skill_name: str, before_score: float, after_score: float, usage_count: int
    ):
    ...
    def calculate_learning_rate(self) -> float:
        ...
        recent_metrics = self.db.query_metrics(
            "learning_analytics", start_date=(datetime.now() - timedelta(days=7)).isoformat()
        )
        ...
        pattern_count = len([m for m in recent_metrics if m["learning_type"] == "pattern_learned"])
        preference_count = len(
            [m for m in recent_metrics if m["learning_type"] == "preference_adapted"]
        )
        mistake_count = len([m for m in recent_metrics if m["learning_type"] == "mistake_learned"])
        behavior_count = len(
            [m for m in recent_metrics if m["learning_type"] == "behavior_adjustment"]
        )

        # Calculate learning rate
        total_learning_events = pattern_count + preference_count + mistake_count + behavior_count
    ...
    def calculate_adaptation_score(self) -> float:
        ...
        metrics = self.db.query_metrics(
            "learning_analytics", start_date=(datetime.now() - timedelta(days=30)).isoformat()
        )
    ...
    def get_learning_report(
        ...
        metrics = self.db.query_metrics(
            "learning_analytics", start_date=start_date, end_date=end_date
        )
        ...
        for metric in metrics:
            ...
            if learning_type not in learning_breakdown:
                learning_breakdown[learning_type] = {"count": 0, "items": [], "avg_confidence": 0.0}
        ...
        for learning_type, data in learning_breakdown.items():
            confidences = [
                item["confidence"] for item in data["items"] if item["confidence"] is not None
            ]
        ...
        return {
            ...
            "insights": self._generate_learning_insights(
                total_patterns, total_preferences, total_mistakes, total_behaviors
            ),
            "top_learnings": self._get_top_learnings(metrics),
        }

    def _generate_learning_insights(
        self, patterns: int, preferences: int, mistakes: int, behaviors: int
    ) -> List[str]:
        ...
        if patterns > 20:
            insights.append(f"📚 {patterns} code patterns learned - improving code generation")

        if preferences > 10:
            insights.append(f"🎯 {preferences} preferences adapted - becoming more personalized")

        if mistakes > 5:
            insights.append(f"💡 Learned from {mistakes} mistakes - continuously improving")

        if behaviors > 5:
            insights.append(f"⚙️  {behaviors} behavior adjustments - adapting to your workflow")
    ...
    def _get_top_learnings(
        self, metrics: List[Dict[str, Any]], limit: int = 5
    ) -> List[Dict[str, Any]]:
        """Get top learnings by confidence"""
        # Sort by confidence
        sorted_metrics = sorted(metrics, key=lambda m: m.get("confidence", 0) or 0, reverse=True)
    ...
    def get_learning_trend(self, days: int = 30) -> List[Dict[str, Any]]:
        ...
        metrics = self.db.query_metrics("learning_analytics", start_date=start_date)
