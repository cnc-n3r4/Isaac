        ...
        try:
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    gpu_count = len(lines)
                    gpu_memory_gb = sum(
                        float(line.split()[0]) / 1024 for line in lines if line.strip()
                    )
            except:
                pass
...
class MachineRegistry:
    ...
    def find_machines_by_tags(self, tags: List[str]) -> List[Machine]:
        ...
        return [m for m in self.machines.values() if m.tags and all(tag in m.tags for tag in tags)]
    ...
    def _save_registry(self):
        ...
        try:
            data = {"machines": [asdict(m) for m in self.machines.values()], "groups": self.groups}
