...
from isaac.arvr.gesture_api import Gesture, GestureAPI, GestureType
...
class MultimodalInput:
    ...
    def is_combined(self) -> bool:
        ...
        active_modalities = sum(
            [self.voice_command is not None, self.gesture is not None, self.gaze_target is not None]
        )
...
class InputPattern:
    ...
    def matches(self, inputs: List[MultimodalInput]) -> bool:
        ...
        if self.voice_keywords:
            voice_texts = [inp.voice_command.text.lower() for inp in inputs if inp.voice_command]
            matched = any(
        ...
        if self.gesture_types:
            gesture_types = [inp.gesture.type for inp in inputs if inp.gesture]
...
class MultimodalInputHandler:
    ...
    def process_voice_input(
        self, text: str, confidence: float = 1.0, gaze_target: Optional[str] = None
    ) -> MultimodalInput:
        """Process voice input"""
        voice_cmd = VoiceCommand(text=text, confidence=confidence, timestamp=time.time())
    ...
    def process_gesture_input(
        self, gesture: Gesture, gaze_target: Optional[str] = None
    ) -> MultimodalInput:
    ...
    def process_combined_input(
        ...
        voice_cmd = None
        if voice_text:
            voice_cmd = VoiceCommand(text=voice_text, confidence=1.0, timestamp=time.time())
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        pattern_matches = sum(1 for inp in self.input_history if "pattern" in inp.metadata)
