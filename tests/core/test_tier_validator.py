...
def test_case_sensitivity(mock_preferences, tier_defaults, monkeypatch):
    """
    Command matching is case-insensitive for convenience.

    Test Coverage:
    - Uppercase vs lowercase commands
    - Expected: Case-insensitive matching
    - Risk if fails: Case variations might not be recognized correctly
    """
    from isaac.core.tier_validator import TierValidator
    ...
    # Case insensitivity (implementation converts to lowercase)
    assert validator.get_tier('ls') == 1, "Lowercase 'ls' should be Tier 1"
    assert validator.get_tier('LS') == 1, "Uppercase 'LS' should also be Tier 1 (case-insensitive)"
    assert validator.get_tier('Get-ChildItem') == 1, "PowerShell cmdlets work regardless of case"
...
def test_tier_defaults_file_exists():
    ...
    # tests/core/test_tier_validator.py -> tests/ -> project_root/
    expected_path = Path(__file__).parent.parent.parent / 'isaac' / 'data' / 'tier_defaults.json'
