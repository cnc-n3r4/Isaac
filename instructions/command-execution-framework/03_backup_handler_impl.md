# Implementation: Backup Handler

## Goal
Create backup command handler that validates user input, confirms destination, and executes backup operations.
...
**Purpose:** Handle `isaac backup <source> to <destination>` commands
- Parse backup arguments (source and destination)
- Validate paths exist
- Confirm operation with user
- Execute backup (copy operation)
- Log result to SessionManager

**User expects:**
```bash
isaac backup my-folder to /mnt/external
AI: Translating 'my-folder'... → /home/user/my-folder
Destination: /mnt/external
Execute backup? (y/n) y
✓ Backup complete: /home/user/my-folder → /mnt/external
```
...
- [ ] File exists at `isaac/commands/backup.py`
- [ ] No syntax errors on import
- [ ] Can instantiate: `handler = BackupHandler(session)`
- [ ] Parse args: `handler._parse_args(["folder", "to", "/dest"])` returns `("folder", "/dest")`
- [ ] Resolve path: `handler._resolve_path("~/Documents")` returns valid Path
- [ ] Execute backup with valid paths completes successfully
...
handler = BackupHandler(session)

# Test argument parsing
source, dest = handler._parse_args(["my-folder", "to", "/tmp"])
print(f"Source: {source}, Dest: {dest}")
# Expected: Source: my-folder, Dest: /tmp

# Test path resolution
from pathlib import Path
resolved = handler._resolve_path(str(Path.home() / "Documents"))
print(f"Resolved: {resolved}")
# Expected: Resolved path to Documents folder

# Test execution (will prompt for confirmation)
result = handler.execute(["test-folder", "to", "/tmp/backup"])
print(result)
# Expected: Prompts for confirmation, executes backup if confirmed
```
...
- ⚠️ **Spaces in paths** - Use `' '.join(args)` to handle multi-word paths. Users should quote paths with spaces.

- ⚠️ **Path resolution** - Always use `Path.resolve()` to get absolute paths. Handle `~` expansion with `os.path.expanduser()`.

- ⚠️ **Destination validation** - For backups, destination parent must exist (we create the final directory/file).

- ⚠️ **Large backups** - `shutil.copytree()` can be slow. Consider showing progress in future iteration.

- ⚠️ **Permissions** - Catch permission errors and provide helpful messages.

- ⚠️ **Logging** - Always log to SessionManager, even on failure. Include status in log message.
